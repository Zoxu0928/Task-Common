...
func (t *TimeMs) UnmarshalJSON(data []byte) (err error) {
	...
	if time_err != nil {
		logger.Error("time type convert error. %s", time_err)
		return
	}
	if !now.IsZero() {
		now = now.Local()
...
func (t TimeMs) MarshalJSON() ([]byte, error) {
	...
	b := make([]byte, 0, len(TIME_FORMART+".000")+2)
	b = append(b, '"')
	b = time.Time(t).AppendFormat(b, TIME_FORMART+".000")
	b = append(b, '"')
	return b, nil
}
...
func (t TimeMs) String() string {
	...
	return t.Time().Format(TIME_FORMART + ".000")
}
